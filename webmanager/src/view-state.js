...
export function parseViewState(search) {
  ...
  return {
    ...
    sessionSort: SESSION_SORTS.has(sessionSort) ? sessionSort : 'name',
  };
}
...
export function serializeViewState(state) {
  ...
  if (state.sessionSort === 'rtt') parameters.set('sort', state.sessionSort);
