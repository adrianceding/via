...
会话状态把 Probe RTT 和停滞与 DATA 容量、排队及写入中负载分开显示。两端互相发布各自实测的发送容量，因此 Manager 可以分别显示上行与下行容量；接收 DATA 仍只表示吞吐，不能作为容量估算。容量样本必须同时满足最小有效 DATA 量和最小观测时长，避免压缩 ACK 突发形成瞬时虚高估算。容量样本经过由 RTT 推导的有界时间后会过期；过期值仍作为最近实测参考显示，但不进入当前总量，线路放置也会回退到默认容量。多 Lane interface 只有在全部 Lane 样本完整且位于同一个并发容量窗口内时才显示总量；否则只保留 Lane 范围和最高值，不把不同时刻测得的样本直接相加。Manager 以 interface 为主要观测对象，每个 interface 最多保留 120 个吞吐样本，每个方向最多显示 12 条 interface 趋势，聚合仍包含全部活动会话。
